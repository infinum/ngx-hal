...
describe('ensureRelationshipRequestDescriptors', () => {
  ...
  it('should return a new array with the same elements if relationship descriptors are passed in', () => {
    const result = ensureRelationshipRequestDescriptors([{ name: 'user' }, { name: 'toys', options: { params: { a: 'c' } } }]);
  ...
  it('should enrich only the elements which do not meet relationship request descriptor needs', () => {
    const result = ensureRelationshipRequestDescriptors([{ name: 'user' }, 'animal', { name: 'toys', options: { params: { a: 'c' } } }]);
    const expectedResult = [{ name: 'user' }, { name: 'animal' }, { name: 'toys', options: { params: { a: 'c' } } }];
    expect(result).toEqual(expectedResult);
  });
});
